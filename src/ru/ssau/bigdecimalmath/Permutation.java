...
class Permutation implements Cloneable{
    ...
    Permutation multiply( final Permutation right ){
        if( images.length != right.images.length ) throw new IllegalArgumentException(
    ...
    public String cyclRep(){
        ...
        for( int c = 0 ; c < allcyc.length ; c++ ){
            for( int e = 0 ; e < allcyc[ c ].length ; e++ ){
                if( e == 0 ) rep += "(" + ( allcyc[ c ][ e ] + 1 - REPOFFSET );
                else if( e == allcyc[ c ].length - 1 ) rep += " " + ( allcyc[ c ][ e ] + 1 - REPOFFSET ) + ")";
                else rep += " " + ( allcyc[ c ][ e ] + 1 - REPOFFSET );
            }
        }

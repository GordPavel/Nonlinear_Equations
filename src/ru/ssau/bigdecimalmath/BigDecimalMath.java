...
public class BigDecimalMath{
     ...
     * Euler's constant.
     *
     * @param precision    The required precision of the result.
     * @param roundingMode THe required rounding mode of the result
     * @return 3.14159...
     * @author Richard J. Mathar
     * @since 2017-07-11
     */
    public static BigDecimal pi( final Integer precision , final RoundingMode roundingMode ){
        return PI.setScale( precision , roundingMode );
    }
     ...
     * @since 2017-07-11
     */
    public static BigDecimal pi( final Integer precision ){
        return PI.setScale( precision , BigDecimal.ROUND_UNNECESSARY );
    }
    ...
    static public BigDecimal pi( final MathContext mc ){
        ...
        if( mc.getPrecision() < PI.precision() ) return PI.round( mc );
        else{
    ...
    static public BigDecimal sqrt( final BigDecimal x , final MathContext mc ){
        ...
        if( x.abs().subtract( new BigDecimal( Math.pow( 10. , -mc.getPrecision() ) ) ).compareTo( BigDecimal.ZERO ) <
            0 ) return BigDecimalMath.scalePrec( BigDecimal.ZERO , mc );
    ...
    static public BigDecimal log( int n , final MathContext mc ){
                /* the value is undefined if x is negative.
                */
        if( n <= 0 ) throw new ArithmeticException( "Cannot take log of negative " + n );
        else if( n == 1 ) return BigDecimal.ZERO;
        else if( n == 2 ){
            if( mc.getPrecision() < LOG2.precision() ) return LOG2.round( mc );
            else{
                                /* Broadhurst <a href="http://arxiv.org/abs/math/9803067">arXiv:math/9803067</a>
            ...
            MathContext mcloc = new MathContext(
                    mc.getPrecision() + 1 + ( int ) ( Math.log10( kmax * 0.693 / 1.098 ) ) );
            ...
            for( int k = 1 ; ; k++ ){
                ...
                if( k % 2 != 0 ) log3 = log3.add( c );
                else log3 = log3.subtract( c );
                pk = pk.multiply( r );
            }
            ...
            MathContext mcloc = new MathContext(
                    mc.getPrecision() + 1 + ( int ) ( Math.log10( kmax * 0.693 / 1.609 ) ) );
    ...
    static public BigDecimal log( final Rational r , final MathContext mc ){
        ...
        else if( r.compareTo( Rational.ONE ) == 0 ) return BigDecimal.ZERO;
        else{
    ...
    static public BigDecimal powRound( final BigDecimal x , final BigInteger n ){
        ...
        else return powRound( x , n.intValue() );
    } /* BigDecimalMath.powRound */
    ...
    static public BigDecimal powRound( final BigDecimal x , final Rational q ){
        /** Special cases: x^1=x and x^0 = 1
         */
        if( q.compareTo( BigInteger.ONE ) == 0 ) return x;
        else if( q.signum() == 0 ) return BigDecimal.ONE;
        else if( q.isInteger() ){
        ...
        if( x.compareTo( BigDecimal.ZERO ) == 0 ) return BigDecimal.ZERO;
            ...
            if( xDbl > 0.8 ){
                ...
                for( int i = 2 ; ; i++ ){
                    ...
                    fac = fac.multiply( new BigInteger( "" + ( 2 * i ) ) ).multiply(
    ...
    static public BigDecimal asin( final BigDecimal x ){
        if( x.compareTo( BigDecimal.ONE ) > 0 || x.compareTo( BigDecimal.ONE.negate() ) < 0 ){
            ...
            for( int i = 1 ; ; i++ ){
                ...
                if( i == 1 ) xpowi = xhighprV;
                else xpowi = multiplyRound( xpowi , xhighprV );
                ...
                resul = resul.add( c );
                                ...
                                */
                if( Math.abs( c.doubleValue() ) < xUlpDbl / 120. ) break;
            }
                        /* sqrt(2*z)*(1+...)
                        */
    ...
    static public BigDecimal Gamma( final Rational q , final MathContext mc ){
        if( q.isBigInteger() ){
            if( q.compareTo( Rational.ZERO ) <= 0 ) throw new ArithmeticException( "Gamma at " + q.toString() );
            else{
                                /* Gamma(n) = (n-1)! */
    ...
    static public BigDecimal mod2pi( BigDecimal x ){
        ...
        int k = ( int ) ( 0.5 * x.doubleValue() / Math.PI );
                ...
                */
        double err2pi;
        if( k != 0 ) err2pi = 0.25 * Math.abs( x.ulp().doubleValue() / k );
        else err2pi = 0.5 * Math.abs( x.ulp().doubleValue() );
    ...
    static public BigDecimal modpi( BigDecimal x ){
        ...
        int k = ( int ) ( x.doubleValue() / Math.PI );
                ...
                */
        double errpi;
        if( k != 0 ) errpi = 0.5 * Math.abs( x.ulp().doubleValue() / k );
        else errpi = 0.5 * Math.abs( x.ulp().doubleValue() );
        ...
        if( res.compareTo( pihalf ) > 0 ) res = res.subtract( onepi );
        else if( res.compareTo( pihalf.negate() ) < 0 ) res = res.add( onepi );
    ...
    static public BigDecimal zeta( final int n , final MathContext mc ){
        if( n <= 0 ) throw new ProviderException( "Not implemented: zeta at negative argument " + n );
        if( n == 1 ) throw new ArithmeticException( "Pole at zeta(1) " );
        ...
        if( n % 2 == 0 ){
            ...
            for( int npr = 0 ; npr <= ( n + 1 ) / 2 ; npr++ ){
                ...
                if( npr % 2 == 0 ) betsum = betsum.add( b );
                else betsum = betsum.subtract( b );
            }
    ...
    static public double zeta1( final int n ){
        ...
        if( n < zmin1.length )
                        /* look it up if available */ return zmin1[ n ];
        else{
    ...
    static public double psi( final double x ){
        ...
        if( x > 2.0 ){
            ...
            final double[] psiT0 = { 9.67672245447621170427e-01 , -4.42763168983592106093e-01 ,
                                     2.58499760955651010624e-01 , -1.63942705442406527504e-01 ,
                                     1.07824050691262365757e-01 , -7.21995612564547109261e-02 ,
                                     4.88042881641431072251e-02 , -3.31611264748473592923e-02 ,
                                     2.25976482322181046596e-02 , -1.54247659049489591388e-02 ,
                                     1.05387916166121753881e-02 , -7.20453438635686824097e-03 ,
                                     4.92678139572985344635e-03 , -3.36980165543932808279e-03 ,
                                     2.30512632673492783694e-03 , -1.57693677143019725927e-03 ,
                                     1.07882520191629658069e-03 , -7.38070938996005129566e-04 ,
                                     5.04953265834602035177e-04 , -3.45468025106307699556e-04 ,
                                     2.36356015640270527924e-04 , -1.61706220919748034494e-04 ,
                                     1.10633727687474109041e-04 , -7.56917958219506591924e-05 ,
                                     5.17857579522208086899e-05 , -3.54300709476596063157e-05 ,
                                     2.42400661186013176527e-05 , -1.65842422718541333752e-05 ,
                                     1.13463845846638498067e-05 , -7.76281766846209442527e-06 ,
                                     5.31106092088986338732e-06 , -3.63365078980104566837e-06 ,
                                     2.48602273312953794890e-06 , -1.70085388543326065825e-06 ,
                                     1.16366753635488427029e-06 , -7.96142543124197040035e-07 ,
                                     5.44694193066944527850e-07 , -3.72661612834382295890e-07 ,
                                     2.54962655202155425666e-07 , -1.74436951177277452181e-07 ,
                                     1.19343948298302427790e-07 , -8.16511518948840884084e-08 ,
                                     5.58629968353217144428e-08 , -3.82196006191749421243e-08 ,
                                     2.61485769519618662795e-08 , -1.78899848649114926515e-08 ,
                                     1.22397314032336619391e-08 , -8.37401629767179054290e-09 ,
    ...
    public static Rational toRational( BigDecimal x ){
                ...
                */
        int s = x.scale();
        if( s > 0 ) return new Rational( x.unscaledValue() , BigInteger.TEN.pow( s ) );
        else return new Rational( x.unscaledValue().multiply( BigInteger.TEN.pow( -s ) ) , BigInteger.ONE );
    } /* toRational */
    ...
    static public BigDecimal multiplyRound( final BigDecimal x , final Rational f ){
        if( f.compareTo( BigInteger.ZERO ) == 0 ) return BigDecimal.ZERO;
        else{
    ...
    static public BigComplex divideRound( final BigInteger n , final BigComplex x ){
                ...
                */
        if( x.im.compareTo( BigDecimal.ZERO ) == 0 ) return new BigComplex( divideRound( n , x.re ) , BigDecimal.ZERO );

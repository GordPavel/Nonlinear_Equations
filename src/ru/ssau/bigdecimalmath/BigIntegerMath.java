...
public class BigIntegerMath{
    ...
    static public BigInteger binomial( final int n , final int k ){
        ...
        BigInteger n2  = bin;
        for( BigInteger i = BigInteger.valueOf( ( long ) ( k - 1 ) ) ; i.compareTo( BigInteger.ONE ) >= 0 ;
             i = i.subtract( BigInteger.ONE ) )
            bin = bin.multiply( n2.subtract( i ) );
        for( BigInteger i = BigInteger.valueOf( ( long ) k ) ; i.compareTo( BigInteger.ONE ) == 1 ;
             i = i.subtract( BigInteger.ONE ) )
            bin = bin.divide( i );
        return ( bin );
    } /* binomial */
    ...
    static public BigInteger isqrt( final BigInteger n ){
        if( n.compareTo( BigInteger.ZERO ) < 0 ) throw new ArithmeticException( "Negative argument " + n.toString() );
    ...
    static public BigInteger iroot( final BigInteger x , final int n ){
        if( x.compareTo( BigInteger.ZERO ) < 0 ) throw new ArithmeticException( "Negative argument " + x.toString() );
        ...
        for( int c = 0 ; c < cL - 1 ; c++ ){
            ...
            for( int r = c + 1 ; r < rL ; r++ ){
                ...
                for( int cpr = c + 1 ; cpr < cL ; cpr++ ){
                    BigInteger tmp = A[ c ][ c ].multiply( A[ r ][ cpr ] ).subtract(
                            A[ c ][ cpr ].multiply( A[ r ][ c ] ) );
                    A[ r ][ cpr ] = tmp;
                }
    ...
    static public Rational centrlFactNumT( int n , int k ){
        if( k > n || k < 0 || ( k % 2 ) != ( n % 2 ) ) return Rational.ZERO;
                        ...
                        /* Proposition 2.1 */
            return centrlFactNumT( n - 2 , k - 2 ).add(
                    centrlFactNumT( n - 2 , k ).multiply( new Rational( k * k , 4 ) ) );
        }
    } /* CentralFactNumT */

...
public class Bernoulli{
    ...
    */
    ...
    protected void set( final int n , final Rational value ){
        final int nindx = n / 2;
        if( nindx < a.size() ) a.set( nindx , value );
        else{
    ...
    public Rational at( int n ){
        if( n == 1 ) return ( new Rational( -1 , 2 ) );
        else if( n % 2 != 0 ) return Rational.ZERO;
        else{
            final int nindx = n / 2;
            if( a.size() <= nindx ){
    ...
    private Rational doubleSum( int n ){
        ...
        for( int k = 0 ; k <= n ; k++ ){
            ...
            for( int j = 0 ; j <= k ; j++ ){
                ...
                if( j % 2 == 0 ) jsum = jsum.add( bin.multiply( jpown ) );
                else jsum = jsum.subtract( bin.multiply( jpown ) );
